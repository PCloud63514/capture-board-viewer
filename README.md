...
오류 메시지 없이 캡처보드가 인식되지 않거나 장치 목록이 비어 있는 경우, 로그 파일을 제출해 주시면 원인을 확인해 드립니다.

### 로그 파일 위치

프로그램은 실행할 때마다 장치 검색 결과를 자동으로 로그 파일에 저장합니다.
...
3. 해당 폴더에서 가장 최근 날짜의 `.log` 파일을 찾습니다.

### 로그 제출 방법

찾은 로그 파일을 아래 이메일에 첨부해서 보내주세요.

📧 **devgraftteam@gmail.com**

메일 작성 시 아래 내용을 함께 알려주시면 더 빠르게 도움드릴 수 있습니다.

- 사용 중인 캡처보드 모델명
- 문제가 발생한 상황 (예: 장치 목록이 비어있음, 특정 장치가 안 보임 등)
